...
namespace ecs
{
    ...
    public:
        ...
        void setActor(std::weak_ptr<AActor> parent);
        std::shared_ptr<AActor> getActor() const;

        AContainer* getContainer() const; //возвратим текущий контейнер
    ...
    public:
        ...
        /// поиск компанента в акторе
        template< class T = AComponent >
        void findComponents(std::vector<std::shared_ptr<T>> &list) const
        {
            const auto objActor = mParent.lock();
            if (objActor)
            {
